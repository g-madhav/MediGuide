...
def load_llm(huggingface_repo_id):
    # FIXED: Pass token correctly and remove problematic parameters
    llm = HuggingFaceEndpoint(
        repo_id=huggingface_repo_id,
        temperature=0.5,
        huggingfacehub_api_token=HF_TOKEN  # ✅ Correct way to pass token
        # Removed model_kwargs to avoid parameter conflicts
    )
    return llm
...
# Display only book name and page number from source documents
print("\nSOURCE DOCUMENTS:")
for i, doc in enumerate(response["source_documents"], 1):
    metadata = doc.metadata
    book_name = metadata.get('source', 'Unknown Book')
    page_number = metadata.get('page', 'Unknown Page')
    print(f"{i}. Book: {book_name}, Page: {page_number}")